...
RUN_NAME = "word2vec_rnn"
REGISTERED_MODEL_NAME = "sentiment-review-rnn"
...
def train_rnn() -> tuple[RNNModel, dict[str, float], dict[str, float]]:
    set_seed()
    ...
    with mlflow.start_run(run_name=RUN_NAME):
        ...
        model = model.to("cpu")
        saved_paths = save_rnn_inference_bundle(
            model=model,
            embedding_matrix=embedding_matrix,
            word2idx=word2idx,
            model_config=MODEL_CONFIG,
        ...
        mlflow.log_artifacts(str(local_model_dir), artifact_path="local_exports/rnn")
