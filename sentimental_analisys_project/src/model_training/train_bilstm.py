...
RUN_NAME = "word2vec_bilstm_attention"
REGISTERED_MODEL_NAME = "sentiment-review-bilstm"

MODEL_DIR_NAME = "bilstm"
RANDOM_STATE = 42
...
def save_bilstm_inference_bundle(
...
def train_bilstm() -> tuple[BiLSTMAttentionClassifier, dict[str, float], dict[str, float]]:
    set_seed()
    ...
    with mlflow.start_run(run_name=RUN_NAME):
        ...
        model = model.to("cpu")
        saved_paths = save_bilstm_inference_bundle(
            model=model,
            embedding_matrix=embedding_matrix,
            word2idx=word2idx,
            model_config=MODEL_CONFIG,
        ...
        mlflow.log_artifacts(str(local_model_dir), artifact_path=f"local_exports/{MODEL_DIR_NAME}")
